...
class State:
    ...
    def graph(self):
        ...
        graph.add_node(pydot.Node('start', shape='plaintext', label='', width=0, height=0))
        ...
        def visit(start):
            ...
            if ids not in visited:
                visited.add(ids)
                graph.add_node(pydot.Node(ids, label=start.name, shape=self.shape, style='bold' if start.final else ''))
                for tran, destinations in start.transitions.items():
                    for end in destinations:
                        visit(end)
                        graph.add_edge(pydot.Edge(ids, id(end), label=tran, labeldistance=2))
                for end in start.epsilon_transitions:
                    visit(end)
                    graph.add_edge(pydot.Edge(ids, id(end), label='ε', labeldistance=2))

        visit(self)
        graph.add_edge(pydot.Edge('start', id(self), label='', style='dashed'))
    ...
    def write_to(self, file_name):
        return self.graph().write_svg(file_name)
